export default function CarouselSlider({ swiper, on }) {
  ...
  on('progress', () => {
    ...
    for (let i = 0; i < swiper.slides.length; i += 1) {
      ...
      let modify = 1;
      if (absProgress > 1) {
        modify = (absProgress - 1) * window.modifyCoefficient + 1;
      }
      // modify = (absProgress - 1) * 0.55 + 1;
      const opacityEls = slideEl.querySelectorAll('.swiper-carousel-animate-opacity');
      const translate = `${slideProgress * modify * window.translateCoefficient}%`;
      // const translate = `${slideProgress * modify * 16}%`;
      ...
      //   opacityEl.style.opacity = 1 - absProgress / 7;
      // });
    }
  });
